...
export async function POST(req: NextRequest) {
  ...
  const { data: vendor } = await supabaseService.from("getaquote_vendors").select("id").eq("id", session.user.id).single();
  if (!vendor) {
  ...
  const { data: request } = await supabaseService.from("getaquote_requests").select("user_id,status").eq("id", request_id).single();
  ...
  const { data: existing } = await supabaseService.from("getaquote_quotes").select("id").eq("request_id", request_id).eq("vendor_id", session.user.id).single();
  if (existing) return NextResponse.json({ error: "You have already submitted a quote on this request" }, { status: 400 });

  const { data, error } = await supabaseService.from("getaquote_quotes").insert({
    request_id,
...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabaseService
    ...
    .select("*, request:getaquote_requests(title,category,location_area,status)")
